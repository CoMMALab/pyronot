"""IK with Collision — CUDA Least-Squares Solver

Inverse Kinematics with collision avoidance using the CUDA LS-IK solver
...
constraints are applied in two JAX-side stages after the CUDA LM loop:
  ...
  1. Winner selection — all ``num_seeds`` CUDA-returned configurations are
     scored with the collision penalty added to the task error.  The least
...
def main():
    """Main function for CUDA LS-IK with collision avoidance."""
    ...
    # Single vmap instance shared across all calls — avoids retracing.
    ...
    def _collision_penalty(cfg, robot, sphere_world):
        ...
        # at_config calls robot.forward_kinematics once; reuse the result for
        # both the floor plane and the sphere instead of calling FK twice.
    ...
    while True:
        ...
        # ── Solve IK with collision constraints (CUDA LS solver) ─────────────
        # Stage 1: CUDA kernel finds 1024 collision-unaware candidates.
        # Stage 2: JAX winner selection scores candidates with collision penalty.
        # Stage 3: Post-CUDA JAX LM (constraint_refine_iters=30) refines winner.
