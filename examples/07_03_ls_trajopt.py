"""Trajectory Optimization with LS TrajOpt.

Robot goes over a wall while avoiding world collisions.
This example reuses the Cartesian seeding pipeline and runs LS TrajOpt as the
...
from pyronot.optimization_engines import LsTrajOptConfig, ScoTrajOptConfig, ls_trajopt
...
def main(
    ...
    print("Running LS TrajOpt...")
    t0 = time.perf_counter()
    best_traj, costs, _ = ls_trajopt(
        init_trajs,
        start_cfg,
        goal_cfg,
        robot,
        ...
        LsTrajOptConfig(
            ...
            n_ls_iters=n_ls_iters,
            n_cg_iters=n_cg_iters,
            cg_tol=1e-4,
            inner_stall_patience=3,
            lambda_init=5e-3,
            w_smooth=w_smooth,
            w_acc=w_acc,
            w_jerk=w_jerk,
