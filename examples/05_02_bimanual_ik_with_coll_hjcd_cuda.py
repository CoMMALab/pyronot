...
Two end-effectors are controlled simultaneously:
  - right_gripper: primary IK target (CUDA coarse + LM phases optimise this EE)
...
-------------------------------------------------
The CUDA kernels optimise only the first EE (right_gripper).  Both EEs are
incorporated in two JAX-side stages:

  1. Winner selection — all refined CUDA configurations are scored with all
...
HJCD-IK explores a much larger seed space (default 256 seeds) than the
...
def main():
    ...
    while True:
        ...
        # Phase 1 (CUDA): 256-seed Hamiltonian coordinate-descent (right arm).
        # Phase 2 (CUDA): Top-K LM refinement on coarse winners (right arm).
        # Stage 3 (JAX):  Winner selection scored with all EE residuals + collision.
        # Stage 4 (JAX):  Post-CUDA LM refinement with full multi-EE + constraint cost.
