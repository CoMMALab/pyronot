"""Bimanual IK with Collision — CUDA HJCD Solver (Baxter)

Bimanual Inverse Kinematics with collision avoidance for a Baxter robot
using the CUDA HJCD-IK solver.
...
constraints are applied in two JAX-side stages after the CUDA two-phase
(coordinate-descent + LM) loop:

  1. Winner selection — all refined CUDA configurations are scored with the
     collision and left-arm IK penalties added to the task error.  The least
     penalised seed is selected as the winner.
...
HJCD-IK explores a much larger seed space (default 1024 seeds) than the
pure LS-IK solver, making it better at finding globally collision-free
bimanual configurations when the local minimum is heavily constrained.
...
def main():
    """Main function for bimanual CUDA HJCD-IK with collision avoidance on Baxter."""
    ...
    while True:
        ...
        # ── Solve bimanual IK with collision constraints (CUDA HJCD solver) ──
        # Phase 1 (CUDA): 1024-seed Hamiltonian coordinate-descent coarse search.
        # Phase 2 (CUDA): Top-K LM refinement on coarse winners.
        # Stage 3 (JAX):  Winner selection scored with collision + left-arm IK.
        # Stage 4 (JAX):  Post-CUDA LM refinement with constraint-augmented cost.
