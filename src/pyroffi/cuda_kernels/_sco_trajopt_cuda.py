"""JAX FFI wrapper for the CUDA SCO TrajOpt kernel.
...
Provides:
  ...
  sco_trajopt_cuda — full SCO trajectory optimisation on the GPU.
      One CUDA thread per trajectory.  Runs the outer SCO loop including
      FK + finite-difference collision Jacobians + inner L-BFGS entirely
      on-device with no Python-side iteration between outer steps.
...
G = 5 fixed collision groups (type-based):
  0: self-collision, 1: world spheres, 2: world capsules,
  3: world boxes, 4: world halfspaces.
"""
...
def _load_and_register() -> None:
    ...
    capsule = _PyCapsule_New(
        ctypes.cast(getattr(lib, "ScoTrajoptCudaFfi"), ctypes.c_void_p),
        b"xla._CUSTOM_CALL_TARGET",
        None,
    )
    jax.ffi.register_ffi_target("sco_trajopt_cuda", capsule, platform="CUDA")


# ---------------------------------------------------------------------------
# World geometry extraction (same logic as _cuda_collision.py)
# ---------------------------------------------------------------------------

def _extract_world_arrays(
    world_geoms,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Flatten world geometry objects into type-separated float32 numpy arrays.

    Returns:
        spheres    — float32 [Ms, 4]    (x, y, z, r)
        capsules   — float32 [Mc, 7]    (x1,y1,z1, x2,y2,z2, r)
        boxes      — float32 [Mb, 15]   (cx,cy,cz, ax1..ax3, ay1..ay3, az1..az3, hl1,hl2,hl3)
        halfspaces — float32 [Mh, 6]    (nx,ny,nz, px,py,pz)
    """
    from pyroffi.collision._geometry import Sphere, Capsule, Box, HalfSpace
    ...
    for wg in world_geoms:
        ...
        if isinstance(wg, Sphere):
            ...
            raise NotImplementedError(
                f"sco_trajopt_cuda: unsupported world geometry type "
...
def sco_trajopt_cuda(
    ...
    *,
    ...
    Drop-in replacement for ``sco_trajopt()`` that runs the full outer SCO
    loop — FK, finite-difference collision Jacobians, and inner L-BFGS — in
    a single tightly-coupled CUDA kernel (one thread per batch trajectory).
