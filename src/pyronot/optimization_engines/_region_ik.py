"""Region-constrained IK sampling via CUDA — LS-IK and SVGD backends.

LS-IK backend (ls_ik_sample_box_region_cuda):
  Two-phase strategy per seed: Levenberg-Marquardt boundary reach followed by
  Brownian shuffling with periodic FK-check corrections.

SVGD backend (svgd_sample_box_region_cuda):
  Particle-based density transport using Stein Variational Gradient Descent
  with RBF kernel repulsion, avoiding boundary clustering that plagues random
  sampling methods.
    ...
    *,
    ...
    Args:
        ...
        target_entropy: If provided, stop when entropy reaches this value (nats).
            Maximum possible entropy is log(entropy_bins^3) ≈ 6.91 for entropy_bins=10.
